...
- `InputMapper` classifies the duration. `App` decides what that action means on the current screen.
- `MqttTransport` moves bounded byte payloads. It does not interpret screens or persist messages.
- `MessagingService` creates/parses messages and applies self-echo/deduplication rules.
- `MessageStore` owns inbox persistence and retention.
- `DeviceConfig` owns persisted settings and room identity.
- `Ui` owns page and selection state. `App` owns cross-component behavior.
...
| UI state | `src/ui/Ui.*` | Current screen, inbox/send/info indices, notices, preset message list |
| Rendering | `src/display/Display.*` | TFT initialization, layouts, text, footer hints, RGB565 accents |
| Device config | `src/config/DeviceConfig.*` | Stable ID, room derivation, settings validation, `fbconfig` NVS, one-time service seeding |
| Setup | `src/setup/SetupPortal.*` | WiFiManager form, room create/join choice, maintenance fields, save flow |
...
| Messaging | `src/messaging/MessagingService.*` | Outgoing IDs/JSON; incoming parse, self-echo rejection, dedupe, persistence |
...
`lib/FriendBoxCore/src/FriendBoxCore.*` contains logic that compiles without Arduino:
...
- inbox replacement-slot selection.

Host tests live under `tests/host/`. Repository checks and build/release helpers live under `scripts/`. Keep new pure rules in `FriendBoxCore` when they do not need GPIO, NVS, Wi-Fi, Arduino `String`, or another ESP32 API.
...
8. Start Wi-Fi, time, UI, MQTT messaging, and OTA services.
9. Open the Idle screen and enter the normal update loop.
...
6. Drain one accepted incoming message into the UI.
7. Poll one button-release event and apply it to the current screen.
...
GPIO LOW/HIGH
  ...
  -> InputMapper
  -> ButtonAction
  -> App::handleInput(current screen, action)
  -> Ui state change / message send / configuration write
  -> Display render
```
...
The current screens are `Idle`, `Inbox`, `Send`, and `Info`. `Ui` owns the selected indexes and temporary notices; `Display` owns every pixel and font/color decision.
...
3. `Message::valid()` enforces schema version 1, lengths, and `type == "text"`.
...
4. `MessagingService` parses and validates JSON in the main loop.
5. It rejects the local sender ID and any message ID already in `MessageStore`.
6. `MessageStore` persists the accepted message as unread.
...
2. Add navigation and side effects in `App::handleInput()`.
3. Add rendering in `Display`.
4. Keep network/storage logic out of the draw function.

### New persistent option

1. Add the value to `Settings`.
2. Load/save it through centralized keys in `DeviceConfig`.
3. Add a setup parameter in `SetupPortal` if user-configurable.
...
3. Route the parsed type in `MessagingService`/`App`.
...
- Hardware drivers report facts; application/UI layers decide meaning.
